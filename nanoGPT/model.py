...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, loss_mask = None, weights = None):
        device = idx.device
        b, t = idx.size()
        ...
        if targets is not None:
            ...
            if weights is None:
                if loss_mask is not None:
                    ...
                    targets_masked[loss_mask == 0] = 0
                    loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets_masked.view(-1), ignore_index=0)
                else:
